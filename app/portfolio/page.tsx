...
export default function PortfolioPage() {
  ...
  return (
    <div className="flex flex-col gap-8 pb-20 w-full max-w-5xl mx-auto pt-10">
      ...
      {!connected ? (
        <div className="w-full p-12 text-center border border-border/40 rounded-3xl 
        ...
        >
          ...
          <p className="text-muted-foreground max-w-md mx-auto">Connect your Solana wallet to view your staking portfolio, balances, and manage unstake tickets.</p>
          <div className="pt-4">
             <CustomConnectButton />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mt-4">
          ...
          >
             <div className="p-6 border border-border/40 rounded-3xl bg-black/40 backdrop-blur-md relative overflow-hidden group">
                 ...
                 <h3 className="font-semibold text-muted-foreground mb-4 text-sm uppercase tracking-wider">Total Staked Balance</h3>
                 <div className="flex items-end gap-2 mb-2">
                    <span className="text-4xl font-bold font-mono tracking-tighter text-white">
                      {isLoading ? "---" : formatLamports(userStats?.stakedBalance || 0)}
                    </span>
                    <span className="text-xl text-primary font-medium mb-1">stSOL</span>
                 </div>
                 <p className="text-sm text-muted-foreground mb-6">
                    ≈ {isLoading ? "---" : ((userStats?.stakedBalance || 0) / 1e9 * 1.02).toFixed(2)} SOL
                 </p>
                 
                 <hr className="border-border/40 mb-6" />
                 
                 <h3 className="font-semibold text-muted-foreground mb-4 text-sm uppercase tracking-wider">Wallet Balance</h3>
                 <div className="flex items-end gap-2">
                    <span className="text-2xl font-bold font-mono tracking-tighter text-white">
                      {isLoading ? "---" : formatLamports(userStats?.solBalance || 0)}
                    </span>
                    <span className="text-muted-foreground font-medium mb-0.5">SOL</span>
                 </div>

                 <div className="mt-8 pt-6 border-t border-border/40">
                    <Link href="/pools" className="w-full inline-flex items-center justify-center whitespace-nowrap rounded-md ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-primary/20 text-primary hover:bg-primary/30 py-3 text-sm font-bold transition-colors">
                      Stake More SOL
                    </Link>
                 </div>
             </div>
          ...
          >
            ...
            <div className="p-6 border border-border/40 rounded-3xl bg-gradient-to-b from-black/0 to-accent/10 mt-8">
               ...
               <p className="text-sm text-muted-foreground leading-relaxed">
                 When you request to unstake your stSOL (via a specific pool), a ticket is created. The protocol processes these tickets by unbinding SOL from validator nodes at the next epoch boundary. Once processed, you must claim the SOL to receive it in your wallet.
               </p>
            </div>
          </motion.div>
        </div>
