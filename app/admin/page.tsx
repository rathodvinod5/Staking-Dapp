  ...
  return (
    <div className="flex flex-col gap-8 pb-20 w-full max-w-4xl mx-auto pt-10">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Manage reserve balances and process unstake claims.
          </p>
        </div>
        ...
        <div className="flex gap-4">
          <button
            ...
            disabled={
              isProcessing || !globalTickets || globalTickets.length === 0
            }
          >
            Process All Pending
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="rounded-xl border bg-purple-900/10 border-purple-500/20 text-card-foreground shadow-sm">
          <div className="flex flex-col space-y-1.5 p-6 pb-2">
            <h3 className="text-sm font-medium text-purple-400 leading-none tracking-tight">
              Reserve Balance
            </h3>
          </div>
        ...
        <div className="rounded-xl border bg-indigo-900/10 border-indigo-500/20 text-card-foreground shadow-sm">
          <div className="flex flex-col space-y-1.5 p-6 pb-2">
            <h3 className="text-sm font-medium text-indigo-400 leading-none tracking-tight">
              Pending Unstakes Total
            </h3>
          </div>
          <div className="p-6 pt-0">
        ...
        <div className="rounded-xl border bg-rose-900/10 border-rose-500/20 text-card-foreground shadow-sm">
          <div className="flex flex-col space-y-1.5 p-6 pb-2">
            <h3 className="text-sm font-medium text-rose-400 leading-none tracking-tight">
              Deficit / Epoch Req
            </h3>
          </div>
          <div className="p-6 pt-0">
      ...
      <div className="rounded-xl border border-border/80 dark:bg-black/40 bg-white/60 shadow-xl dark:shadow-none backdrop-blur-xl text-card-foreground">
        <div className="flex flex-col space-y-1.5 p-6">
          ...
          <p className="text-sm text-muted-foreground">
            Users waiting for their SOL from the protocol reserve.
          </p>
        </div>
        <div className="p-6 pt-0">
          {isLoadingTickets ? (
            <div className="py-12 text-center text-muted-foreground animate-pulse">
              Loading all protocol tickets...
            </div>
                  ...
                  {globalTickets?.map((ticket) => (
                    ...
                    >
                      ...
                      <td className="p-4 font-mono text-primary/80">
                        {shortenAddress(ticket.user, 6)}
                      </td>
                      <td className="p-4 font-mono text-right font-bold">
                        {formatLamports(ticket.amount)}
                      </td>
      ...
      <div className="text-sm text-center text-muted-foreground/60">
        <p>
          Warning: Administrative actions directly modify protocol state and
          reserves.
        </p>
        <p>
          Do NOT process unstakes if reserve balance is less than the requested
          ticket amount.
        </p>
      </div>
    </div>
